 ...
 * ------------------------------------------------------------------
 * 2020-07-21 Additions (Vinz68)
 *  - Initialized the "SPI Flash File System" , so files can be uploaded via Arduino IDE
 *  - more serial output, so we can see what is going on
 *  - and version raised to v0.2
 *  
...
void setup()
{
  Serial.println("Initialize Heltec and show Logo...");
  Heltec.begin(true /*DisplayEnable Enable*/, false /*LoRa Enable*/, true /*Serial Enable*/);
    
  Serial.begin(115200);

  if (!SPIFFS.begin(true))
    Serial.println("SPI Flash File System (SPIFFS) NOT initialized!");
  else 
    Serial.println("SPI Flash File System (SPIFFS) initialized.");  
